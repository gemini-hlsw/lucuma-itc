...
/**
 * This is a unit test for GMOS imaging mode in time and count mode for the legacy ITC, 
 * ensuring all possible combinations of parameters can be parsed. The ITC may still return an
 * error but we want to ensure it can parse the values.
 */
class LegacyITCGmosImgExpTimeSuite extends FunSuite with CommonITCLegacySuite:
