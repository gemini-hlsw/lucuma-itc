...
case class LocalItc(classLoader: ClassLoader):
  ...
  /**
   * This method does a call to the method ItcCalculation.calculateCharts via reflection. This is
   * done because the itc-server runs on scala 3 while the ItcCalculation method is based on scala 2
   *
   ...
   * essentially the same as if ITC were a server accepting json and responding json
   */
  def calculate(jsonParams: String): Either[String, ExposureTimeRemoteResult] =
