...
import lucuma.odb.api.model.MagnitudeModel
...
object ConfigurationAlternativesSchema {
  ...
  def SuccessItcResultType[F[_]]: ObjectType[ItcRepo[F], Itc.Result.Success] =
    ObjectType(
      name     = "ItcSuccess",
      interfaces  = List(PossibleInterface.apply[ItcRepo[F], Itc.Result.Success](ItcResultType[F])),
      fieldsFn = () => fields(
  ...
  def SourceTooBrightType[F[_]]: ObjectType[ItcRepo[F], Itc.Result.SourceTooBright] =
    ObjectType(
      name     = "ItcError",
      interfaces  = List(PossibleInterface.apply[ItcRepo[F], Itc.Result.SourceTooBright](ItcResultType[F])),
      fieldsFn = () => fields(
  ...
  def ItcResultType[F[_]]: InterfaceType[ItcRepo[F], Itc.Result] =
    InterfaceType[ItcRepo[F], Itc.Result](
      name         = "ItcResult",
      description  = "Possible itc result",
      fields[ItcRepo[F], Itc.Result](
        Field(
          name        = "resultType",
          fieldType   = StringType,
      ...
      PossibleObject[ItcRepo[F], Itc.Result](SourceTooBrightType[F]),
      PossibleObject[ItcRepo[F], Itc.Result](SuccessItcResultType[F])
    ))
    // UnionType(
    //   name        = "Itc Result",
  ...
  def ObservingModeSpectroscopyType[F[_]]: ObjectType[ItcRepo[F], ObservingMode.Spectroscopy] =
    ObjectType(
  ...
  def ResultSpectroscopyType[F[_]]: ObjectType[ItcRepo[F], Result.Spectroscopy] =
    ObjectType(
      name     = "Result",
      fieldsFn = () => fields(
  ...
  def SpectroscopyResultsType[F[_]]: ObjectType[ItcRepo[F], SpectroscopyResults]=
    ObjectType(
