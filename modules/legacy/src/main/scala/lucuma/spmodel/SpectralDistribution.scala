...
final case class EmissionLine(wavelength: Wavelength, width: EmissionLine.Width, flux: EmissionLine.Irradiance, continuum: EmissionLine.SpectralIrradiance) extends SpectralDistribution

object EmissionLine {
  type KPS = (Kilo %* Meter) %/ Second
  type WSM = Watt %/ (Meter %^ 2)
  type WSMM = WSM %/ Micrometer

  type Width = Quantity[numeric.PosBigDecimal, KPS]
  type Irradiance = Quantity[numeric.PosBigDecimal, WSM]
  type SpectralIrradiance = Quantity[numeric.PosBigDecimal, WSMM]
}
