...
import lucuma.spmodel.{AuxFileSpectrum, UserDefinedSpectrum, SpectralDistribution}
...
class ItcServiceImpl extends ItcService {
  ...
  def calculate(p: ItcParameters, headless: Boolean): Result = try {
    ...
    p.observation.calculationMethod match {
      ...
      case _: Spectroscopy  => calculateSpectroscopy(p, headless)
    }
  ...
  private def calculateImaging(p: ItcParameters): Result =
    p.instrument match {
      ...
      case i: GmosSouthParameters              => imagingResult        (new GmosRecipe(p, i))
      case i: GmosNorthParameters              => imagingResult        (new GmosRecipe(p, i))
  ...
  private def calculateSpectroscopy(p: ItcParameters, headless: Boolean): Result =
    p.instrument match {
      ...
      case i: GmosNorthParameters              => spectroscopyResult   (new GmosRecipe(p, i),       headless )
      case i: GmosSouthParameters              => spectroscopyResult   (new GmosRecipe(p, i),       headless )
