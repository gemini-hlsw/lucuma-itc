...
object Warning {
  ...
  def collectGenericWarnings(r: Result): List[ItcWarning] = {
    r.instrument.warnings.asScala.flatMap(_.warning(r)).toList
  }

  def collectWarnings(r: ImagingResult): List[ItcWarning] = {
    collectGenericWarnings(r) ++ r.instrument.imagingWarnings(r).asScala
  }

  def collectWarnings(r: SpectroscopyResult): List[ItcWarning] = {
    collectGenericWarnings(r) ++ r.instrument.spectroscopyWarnings(r).asScala
  }
