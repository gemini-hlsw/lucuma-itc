...
// import java.util. { ArrayList, List => List }
// import lucuma.itc.model.syntax.instrument._
...
object Recipe {
  ...
  //   val data: List[SpcSeriesData] = List(
  //     SpcSeriesData(SignalData,     "Signal",           result.specS2N(index).getSignalSpectrum.getData),
  //     SpcSeriesData(BackgroundData, "SQRT(Background)", result.specS2N(index).getBackgroundSpectrum.getData)
  //   )
  ...
  //   val data: List[SpcSeriesData] = List(
  //     SpcSeriesData(SingleS2NData, "Single Exp S/N", result.specS2N(index).getExpS2NSpectrum.getData),
  //     SpcSeriesData(FinalS2NData,  "Final S/N  ",    result.specS2N(index).getFinalS2NSpectrum.getData)
  //   )
  ...
  def serviceResult(r: SpectroscopyResult, charts: List[SpcChartData], headless: Boolean): ItcSpectroscopyResult =
  ...
  def serviceGroupedResult(r: SpectroscopyResult, charts: List[List[SpcChartData]], headless: Boolean): ItcSpectroscopyResult =
    ItcSpectroscopyResult(
  ...
  def serviceGroupedResult(rs: Array[SpectroscopyResult], charts: List[List[SpcChartData]], headless: Boolean): ItcSpectroscopyResult = {
    ItcSpectroscopyResult(
