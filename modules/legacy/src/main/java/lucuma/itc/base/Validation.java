...
public final class Validation {
    ...
    private static void checkRedshift(final SourceDefinition source) {
        if (source.redshift().z().toDouble() <= -0.9) {
    ...
    private static void checkGaussianFwhm(final SpatialProfile sp) {
        if (sp instanceof GaussianSource) {
            if (((GaussianSource) sp).fwhm().toDoubleDegrees() / 3600 < 0.1) {
    ...
    private static void checkElineWidth(final Instrument instrument, final SourceDefinition source) {
        ...
        if (source.distribution() instanceof EmissionLine) {
            ...
            final double maxWidth = ITCConstants.C / (eLine.wavelength().nanometer().toDouble() * (1.0 + source.redshift().z().toDouble() / resolution));

            if (eLine.width().toDouble() < maxWidth) {
                throw new IllegalArgumentException(
                    String.format(
